<?php
/**
 * Zend2 Framework
 *
 * LICENSE
 *
 * This source file is subject to the new BSD license that is bundled
 * with this package in the file LICENSE.txt.
 * It is also available through the world-wide-web at this URL:
 * http://framework.zend.com/license/new-bsd
 * If you did not receive a copy of the license and are unable to
 * obtain it through the world-wide-web, please send an email
 * to license@zend.com so we can send you a copy immediately.
 *
 * @category   Zend2
 * @package    Zend2_Gdata
 * @subpackage GApps
 * @copyright  Copyright (c) 2005-2012 Zend2 Technologies USA Inc. (http://www.zend.com)
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */

namespace Zend2\GData\GApps;

/**
 * Data model for a collection of Google Apps email list recipient entries,
 * usually provided by the Google Apps servers.
 *
 * For information on requesting this feed from a server, see the Google
 * Apps service class, Zend2_Gdata_GApps.
 *
 * @uses       \Zend2\GData\Feed
 * @uses       \Zend2\GData\GApps\EmailListRecipientEntry
 * @category   Zend2
 * @package    Zend2_Gdata
 * @subpackage GApps
 * @copyright  Copyright (c) 2005-2012 Zend2 Technologies USA Inc. (http://www.zend.com)
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class EmailListRecipientFeed extends \Zend2\GData\Feed
{

    protected $_entryClassName = '\Zend2\GData\GApps\EmailListRecipientEntry';
    protected $_feedClassName = '\Zend2\GData\GApps\EmailListRecipientFeed';

}
