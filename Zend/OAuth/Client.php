...
use Zend2\Http\Request;

/**
 * @uses       Zend2\Http\Client
 * @uses       Zend2\OAuth\OAuth
 * @uses       Zend2\OAuth\Config\StandardConfig
 * @uses       Zend2\OAuth\Exception
 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class Client extends \Zend2\Http\Client
{
    /**
     ...
     * of Zend2_OAuth_Config; it's not a Zend2_Config instance since that level
     * of abstraction is unnecessary and doesn't let me escape the accessors
     * and mutators anyway!
     *
     * @var Zend2\OAuth\Config
     */
     ...
     * just a typical Zend2_HTTP_Client subclass with some OAuth icing to
     * assist in automating OAuth parameter generation, addition and
     * cryptographioc signing of requests.
     *
     * @param  array $oauthOptions
     * @param  string $uri
     * @param  array|\Zend2\Config\Config $config
     * @return void
     */
    public function __construct($oauthOptions, $uri = null, $config = null)
    {
        ...
        if ($oauthOptions !== null) {
            if ($oauthOptions instanceof \Zend2\Config\Config) {
   ...
   /**
    ...
    /**
     * Same as Zend2_HTTP_Client::setMethod() except it also creates an
     ...
     *
     * @return void
     * @throws Zend2\OAuth\Exception If POSTBODY scheme requested, but GET request method used; or if invalid request scheme provided
     */
